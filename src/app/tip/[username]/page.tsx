...
import {
  Zap, ArrowLeft, Copy, ExternalLink, CheckCircle,
  Sparkles, Star, Shield, RefreshCw, Share2, Users, TrendingUp
} from 'lucide-react';
...
/* ── Types ── */
type Token = 'SOL' | 'USDC';
interface TipResult { txHash: string; message: string; amount: number; token: Token; }
interface Supporter { name: string; amount: number; message?: string; timestamp: string; }
...
function TipButton({ label, selected, onClick }: { label: string; selected: boolean; onClick: () => void }) {
  return (
    ...
    >
      {selected && (
        <span className="absolute inset-0 rounded-2xl animate-pulse" style={{ background: 'rgba(153,69,255,0.15)' }} />
      )}
      {label}
    </motion.button>
  );
...
export default function TipPage() {
  ...
  // Load supporters from tip history
  useEffect(() => {
    if (!creator) return;
    const history = creator.tipHistory ?? [];
    const mapped: Supporter[] = history.slice().reverse().map((t: { amount: number; token: string; timestamp: string; message?: string }) => ({
      name: 'Anonymous',
      amount: t.amount,
      message: t.message,
  ...
  const connectWallet = async () => {
    ...
    } catch { setTipError('Wallet connection cancelled.'); }
  ...
  const sendTip = async () => {
    ...
    setSending(true);
    try {
      const postRes = await fetch(`/api/actions/tip/${username}?amount=${amount}&token=${token}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ account: walletAddress }),
      });
      const { transaction } = await postRes.json();
      ...
      const recordRes = await fetch('/api/tips', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username, amount, token, txHash: signature, message }),
      });
      const recordData = await recordRes.json();
      const tipResult: TipResult = {
        txHash: signature,
        message: recordData.thankYouMessage || `Thank you so much for the ${amount} ${token} tip! 🙏`,
        amount,
        token,
      };
      setResult(tipResult);
      ...
      setConfetti(true);
      // Add to supporters wall instantly
      setSupporters(prev => [{ name: walletAddress.slice(0,6), amount, message, timestamp: 'just now' }, ...prev]);
  ...
  return (
    <div className="min-h-screen grid-bg" style={{ background: '#080810' }}>
      ...
      {/* Nav */}
      <nav className="relative z-10 flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        {walletConnected ? (
          ...
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all btn-secondary"
          >
            {connectingWallet ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Shield className="w-3.5 h-3.5" />}
      ...
      <main className="relative z-10 max-w-lg mx-auto px-4 py-8">
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-3 mt-6">
            {[
              { label: 'Total Earned', value: `◎ ${creator.totalTips?.toFixed(2) ?? '0.00'}`, icon: '💎' },
              { label: 'Supporters', value: `${supporters.length || creator.tipCount || 0}`, icon: '🫶' },
        ...
        <div className="flex gap-1 p-1 rounded-2xl mb-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
          {[{ key: 'tip', label: '⚡ Send Tip', icon: Zap }, { key: 'supporters', label: `🫶 Supporters (${supporters.length})`, icon: Users }].map(tab => (
            <button
              key={tab.key}
          ...
          {activeTab === 'tip' ? (
            ...
            >
              ...
              <div className="relative rounded-3xl p-6 noise" style={{
                ...
                {/* Token toggle */}
                <div className="flex items-center gap-2 mb-5 p-1 rounded-2xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  {(['SOL','USDC'] as Token[]).map(t => (
                    <button key={t} onClick={() => { setToken(t); setSelectedPreset(1); setCustomAmount(''); }}
                      className="flex-1 py-2.5 rounded-xl font-bold text-sm transition-all"
                      style={token === t ? {
                        background: 'linear-gradient(135deg, #9945FF, #7B2FFF)',
                        color: '#fff', boxShadow: '0 0 20px #9945FF50',
                      } : { color: '#888' }}
                    >{t}</button>
                  ))}
                </div>
                ...
                {tipError && (
                  <div className="flex items-center gap-2 p-3 rounded-xl mb-4" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.25)' }}>
                ...
                >
                  {!sending && <div className="absolute inset-0" style={{ background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.06), transparent)', animation: 'shimmer 2.5s infinite' }} />}
                  {sending ? (
                    <span className="flex items-center justify-center gap-3"><RefreshCw className="w-5 h-5 animate-spin" /> Sending on Solana…</span>
                  ) : !walletConnected ? (
                    <span className="flex items-center justify-center gap-2"><Shield className="w-5 h-5" /> Connect Phantom to Tip</span>
                  ) : (
                    <span className="flex items-center justify-center gap-2"><Zap className="w-5 h-5" /> Send {amount > 0 ? `${token === 'SOL' ? '◎' : '$'}${amount}` : ''} {token}</span>
                  )}
                </motion.button>
            ...
            >
              <SupporterWall supporters={supporters} creatorName={creator.name} />
            </motion.div>
          )}
        </AnimatePresence>
