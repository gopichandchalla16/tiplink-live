...
const PERSONALITIES = [
  ...
  { key: 'hype', emoji: '🔥', label: 'Hype', desc: 'High-energy & excited', color: '#FF6B35',
    sample: "LFG!! That tip just MADE MY DAY!! We're building together and I'm SO HYPE!! 🔥🚀💯" },
...
export default function CreatePage() {
  ...
  const enhanceBio = async () => {
    ...
    try {
      ...
      const { enhancedBio } = await res.json();
      if (enhancedBio) setBio(enhancedBio);
    } catch { /* silent */ }
  ...
  const createProfile = async () => {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json(); throw new Error(d.error ?? 'Failed to create profile'); }
      const baseUrl = window.location.origin;
      setTipUrl(`${baseUrl}/tip/${username.toLowerCase()}`);
      setStep(4);
      ...
      <div className="flex items-center gap-2 mb-8 relative z-10">
        {[1,2,3].map(s => (
          <div key={s} className="rounded-full transition-all duration-300"
            style={step > s ? { width: 8, height: 8, background: '#22C55E' }
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              className="rounded-3xl p-8 noise"
              style={{ background: 'linear-gradient(145deg, rgba(15,15,26,0.97), rgba(10,10,20,0.99))', border: '1px solid rgba(153,69,255,0.2)' }}
            >
              <div className="flex justify-center mb-6">
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center animate-pulse-glow" style={{ background: 'linear-gradient(135deg, #9945FF, #7B2FFF)' }}>
                  <Shield className="w-8 h-8 text-white" />
                </div>
              </div>
              <h1 className="text-2xl font-extrabold text-white text-center mb-2" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>Connect your wallet</h1>
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              ...
              style={{ background: 'linear-gradient(145deg, rgba(15,15,26,0.97), rgba(10,10,20,0.99))', border: '1px solid rgba(153,69,255,0.2)' }}
            >
              <h1 className="text-2xl font-extrabold text-white mb-1" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>Create your profile</h1>
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  ...
                  <input value={username} onChange={e => setUsername(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g,''))}
                    ...
                    className="w-full pl-8 pr-10 py-3 rounded-2xl text-white placeholder-gray-600 text-sm font-semibold focus:outline-none transition-all"
                ...
                <div className="flex flex-wrap gap-2">
                  {CATEGORIES.map(c => (
                    <button key={c} onClick={() => setCategory(c)}
                      ...
                      style={category === c
                        ? { background: 'linear-gradient(135deg,#9945FF,#7B2FFF)', color:'#fff', boxShadow:'0 0 16px #9945FF40' }
          ...
          {step === 3 && (
            <motion.div key="s3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              ...
              style={{ background: 'linear-gradient(145deg, rgba(15,15,26,0.97), rgba(10,10,20,0.99))', border: '1px solid rgba(153,69,255,0.2)' }}
            >
              <h1 className="text-2xl font-extrabold text-white mb-2" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>Choose your personality</h1>
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                {PERSONALITIES.map(p => (
                  <button key={p.key} onClick={() => setPersonality(p.key)}
                    ...
                    style={personality === p.key
                      ? { background: `${p.color}15`, border: `2px solid ${p.color}60`, boxShadow: `0 0 20px ${p.color}25` }
                      : { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}
                  >
                    {personality === p.key && (
                      <div className="absolute top-2 right-2 w-5 h-5 rounded-full flex items-center justify-center" style={{ background: p.color }}>
          ...
          {step === 4 && (
            <motion.div key="s4" initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
              className="rounded-3xl p-8 text-center noise"
              style={{ background: 'linear-gradient(145deg, rgba(15,15,26,0.97), rgba(10,10,20,0.99))', border: '1px solid rgba(34,197,94,0.3)' }}
            >
              ...
              <h1 className="text-3xl font-extrabold text-white mb-2" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>You&apos;re live! 🎉</h1>
              ...
              {tipUrl && (
                <div className="flex justify-center mb-6">
                  <div className="inline-block bg-white p-4 rounded-3xl">
                    <QRCodeSVG value={tipUrl} size={180} level="H" fgColor="#9945FF" bgColor="#FFFFFF" />
                  </div>
                </div>
              )}
              <div className="rounded-2xl p-3 mb-4 flex items-center gap-2" style={{ background: 'rgba(153,69,255,0.08)', border: '1px solid rgba(153,69,255,0.25)' }}>
                ...
                <button onClick={() => navigator.clipboard.writeText(tipUrl)} className="text-gray-400 hover:text-white transition-colors">
                  <Copy className="w-4 h-4" />
                </button>
              </div>
              <p className="text-xs text-gray-500 mb-6">Scan to tip on mobile</p>
              <div className="flex gap-3">
                <Link href={tipUrl.replace(window.location.origin,'')} className="btn-primary flex-1">
